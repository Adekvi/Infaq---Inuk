<x-utama.layout.main title="Superadmin | Struktur Pengurus">
            ...
            <div class="col-lg-12 mb-4 order-0">
                <div class="pasien-bpjs">
                    <div class="card-title">
                        <h5 style="margin-bottom: 20px"><strong>Struktur Pengurus</strong></h5>
                    </div>
                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="mb-1" style="display: flex; justify-content: space-between">
                                <h5>Tabel Data</h5>
                                <a href="{{ url('superadmin/landing/struktur-pengurus/tambah-data') }}"
                        ...
                        <div class="card-body">
                            <div
                                ...
                                <form method="GET" action="{{ route('superadmin.landing.struktur') }}"
                                    ...
                                    <div class="d-flex gap-2 align-items-end">
                                        <div>
                                            <a href="{{ route('superadmin.landing.struktur') }}"
                            ...
                            <div class="row">
                                ...
                                @else
                                                ...
                                                <div class="card-body">
                                                    <h6 class="card-title mb-2">
                                                        <div class="d-flex justify-content-between">
                                                            ...
                                                            <form method="POST"
                                                                action="{{ url('superadmin/landing/struktur-status') }}">
                                                                @csrf
                                                    ...
                                                    <div class="mb-1 d-flex">
                                                        <div style="width: 120px;"><strong>Pengurus</strong>
                                                        </div>
                                                    ...
                                                    <div class="mb-1 d-flex">
                                                        <div style="width: 120px;"><strong>Manajemen</strong>
                                                        </div>
                                                        <span class="me-1">:</span>
                                                        <div>
                                                            {{ $item->masamanajemen ?? '-' }}
                                    ...
                                    <div class="col-12 mt-3 d-flex justify-content-end">
                                        {{ $struktur->appends(['entries' => $entries])->links() }}
                                    </div>
                                @endif
                            </div>
                        </div>
    ...
    @include('superadmin.landing.struktur.modal.detail')
    @include('superadmin.landing.struktur.hapus')
    ...
    @push('js')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                const filterYearStart = document.getElementById('filter_year_start');
                const filterYearEnd = document.getElementById('filter_year_end');

                async function fetchAndSetEndYear(startYear) {
                    if (!startYear) {
                        // Jika kosong, reset dropdown end year
                        filterYearEnd.value = '';
                        for (const option of filterYearEnd.options) {
                    ...
                    try {
                        const response = await fetch(`/struktur/get-end-year?start_year=${startYear}`);
                        const data = await response.json();

                        if (data.max_end_year) {
                            // Set value dan disable option yang lebih kecil dari max_end_year
                            filterYearEnd.value = data.max_end_year;

                            for (const option of filterYearEnd.options) {
                                option.disabled = parseInt(option.value) < startYear || parseInt(option.value) >
                                    data.max_end_year;
                            }
                        } else {
                            // Jika tidak ada data, reset dropdown end year
                            filterYearEnd.value = '';
                            for (const option of filterYearEnd.options) {
                                option.disabled = false;
                            }
                        ...
                        console.error('Error fetching end year:', error);
                    }
                }

                filterYearStart.addEventListener('change', function() {
                    const startYear = parseInt(filterYearStart.value);
                    fetchAndSetEndYear(startYear);
                });

                // Optional: jalankan sync saat halaman load jika sudah ada nilai start_year
                if (filterYearStart.value) {
                    fetchAndSetEndYear(parseInt(filterYearStart.value));
                }
            });
        </script>
    @endpush
