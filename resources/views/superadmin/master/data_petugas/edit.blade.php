            ...
            <div class="col-lg-12 mb-4 order-0">
                <div class="card-title">
                    ...
                    <div class="mb-1" style="display: flex; justify-content: start">
                        <code>Edit Petugas</code>
                    </div>
                </div>
                <div class="card">
                    <div class="card-body">
                        <form action="{{ url('superadmin/master-data/petugas-dataPetugas/edit/' . $petugas->id) }}"
                                        ...
                                        <select name="kelurahans[]" id="id_kelurahan" class="form-select mt-2 mb-2"
                                            ...
                                            @foreach ($kelurahans as $item)
                                                <option value="{{ $item->id }}"
                                                    {{ $petugas->kelurahans->contains('id', $item->id) ? 'selected' : '' }}>
                                                    {{ $item->nama_kelurahan }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="nama_petugas" id="nama_petugas"
                                            class="form-control mt-2 mb-2" placeholder="Nama Petugas"
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="longitude" id="longitude"
                                            ...
                                            value="{{ $petugas->longitude }}">
                                    </div>
                                </div>
                            </div>
                            <button type="submit" class="btn btn-primary mt-3 mr-2">Simpan Perubahan</button>
                        </form>
                    </div>
                </div>
            </div>
    ...
    @push('js')
        ...
        <script>
            $(document).ready(function() {
                ...
                // Simpan data kelurahan dari PHP ke JavaScript untuk akses cepat
                const kelurahanData = @json(
                    $petugas->kelurahans->mapWithKeys(function ($kelurahan) {
                        return [$kelurahan->id => ['RW' => $kelurahan->pivot->RW, 'RT' => $kelurahan->pivot->RT]];
                    }));
                ...
                $('#id_kelurahan').on('change', function() {
                    ...
                    if (selectedKelurahans.length > 0) {
                        selectedKelurahans.forEach((id, index) => {
                            ...
                            // Ambil data RW dan RT dari kelurahanData jika ada
                            const rw = kelurahanData[id]?.RW || '';
                            const rt = kelurahanData[id]?.RT || '';
                            ...
                            <div class="rw-rt-group mb-3">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <input type="text" name="kelurahans[${index}][RW]" class="form-control mb-2" placeholder="RW" value="${rw}">
                                    </div>
                                    <div class="col-md-6">
                                        <input type="text" name="kelurahans[${index}][RT]" class="form-control mb-2" placeholder="RT" value="${rt}">
                                    </div>
                                </div>
                            </div>
                        `);
                ...
                // Trigger change untuk menampilkan RW/RT awal
